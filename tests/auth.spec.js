...
test.describe('Módulo de Autenticación', () => {
  ...
  test('5. Cerrar sesión exitosamente (LogOut)', async ({ request }) => {
    expect(token).not.toBe('');
    ...
    // Validamos que responda 200 OK (Sesión cerrada en el servidor)
    expect(response.status()).toBe(200);

    console.log('--- LOGOUT EXITOSO ---');
    console.log('El token ha sido invalidado correctamente por el servidor.');
  });
});